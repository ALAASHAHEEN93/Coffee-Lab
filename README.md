...
Follow steps 1 and 2 from above, the docker-compose file will automatically use the .env file in your project root
...
Invoke the next build script by running pnpm build or npm run build in your project root. This creates a .next directory with a production-ready admin bundle.
Finally run pnpm start or npm run start to run Node in production and serve Payload from the .build directory.
